        ...
        @if($dewasa->count())
            <div class="overflow-x-auto">
                <table class="table table-compact w-full">
                    ...
                    @foreach($dewasa as $d)
                        <tr>
                            <td>{{ $d->tanggal_periksa->format('d/m/Y') }}</td>
                            <td>{{ $d->warga->nik }}</td>
                            <td>{{ $d->warga->nama }}</td>
        ...
        @if($lansia->count())
            <div class="overflow-x-auto">
                <table class="table table-compact w-full">
                    ...
                    @foreach($lansia as $l)
                        <tr>
                            <td>{{ $l->tanggal_periksa->format('d/m/Y') }}</td>
                            <td>{{ $l->warga->nik }}</td>
                            <td>{{ $l->warga->nama }}</td>
                            <td><span class="badge badge-lg {{ $l->aks_kategori == 'M' ? 'badge-success' : 'badge-warning' }}">{{ $l->aks_kategori_text }}</span></td>
